...
export async function dashboard(_request: Request, response: Response): Promise<void> { ok(response, await getDashboard()) }
export async function activities(request: Request, response: Response): Promise<void> { ok(response, await listActivities(typeof request.query.documentId === "string" ? request.query.documentId : undefined)) }
...
export async function find(request: Request, response: Response): Promise<void> { ok(response, await findDocument(routeParam(request.params.id))) }
export async function trace(request: Request, response: Response): Promise<void> { ok(response, await getTrace(routeParam(request.params.id))) }
export async function impact(request: Request, response: Response): Promise<void> { ok(response, await assessImpact(routeParam(request.params.id), documentImpactSchema.parse(request.body).masterData)) }
...
export async function update(request: Request, response: Response): Promise<void> { ok(response, await updateDocument(routeParam(request.params.id), request.body, request.headers), "单据已保存") }
export async function remove(request: Request, response: Response): Promise<void> { await removeDocument(routeParam(request.params.id)); ok(response, null, "单据已删除") }
export async function action(request: Request, response: Response): Promise<void> {
  ...
  ok(response, await executeAction(routeParam(request.params.id), actionValue, request.headers, request.body?.comment), "流程操作成功")
}
