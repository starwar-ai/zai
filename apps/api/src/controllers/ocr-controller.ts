...
const recognitionTypeSchema = z.enum(["PAYMENT", "INVOICE", "NAVIGATION_ROUTE"])
...
export async function postOcrRecognition(request: Request, response: Response): Promise<void> { const input = recognizeSchema.parse(request.body); ok(response, await recognizeOcr(requestUser(request).userId, input), input.recognitionType === "INVOICE" ? "发票识别完成" : input.recognitionType === "NAVIGATION_ROUTE" ? "导航截图识别完成" : "支付截图识别完成") }
