...
function firstMatch(text: string, joined: string, compact: string, patterns: RegExp[]): string | undefined {
  for (const pattern of patterns) {
    const match = joined.match(pattern) ?? text.match(pattern) ?? compact.match(pattern)
...
export function parseTrainTicketText(text: string): OcrTrainTicketData {
  ...
  const dateTimeMatch = joined.match(/(\d{4}年\d{2}月\d{2}日)\s+(\d{2}:\d{2})[开開]/)
    ?? text.match(/(\d{4}年\d{2}月\d{2}日)\s+(\d{2}:\d{2})[开開]/)
    ?? compact.match(/(\d{4}年\d{2}月\d{2}日)[^\d]{0,30}(\d{2}:\d{2})[开開]/)
  const passengerMatch = text.match(/\d{6,10}\*{4,8}\d{4}\s+([\u3400-\u9fff]{2,5})/) ?? text.match(/\d{6}[\d*]{6,8}\d{4}\s{2,}([\u3400-\u9fff]{2,5})/)
  const value = (patterns: RegExp[]) => firstMatch(text, joined, compact, patterns)
  return {
    ...
    passengerId: value([/(\d{6,10}\*{4,8}\d{4})/, /(\d{17}[\dXx])(?=[\u3400-\u9fff]{2,5})/]),
