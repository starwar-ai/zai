...
export function llmProviderConfig(provider: string): LlmProviderConfig | undefined {
  ...
  return configs[provider.trim().toLowerCase()]
}

export function configuredLlmProviders(): LlmProviderConfig[] {
  return (process.env.LLM_PROVIDER_ORDER || "openai").split(",").map(llmProviderConfig).filter((item): item is LlmProviderConfig => Boolean(item))
}
