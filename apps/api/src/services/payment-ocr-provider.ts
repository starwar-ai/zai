...
export async function recognizePaymentScreenshot(input: OcrRecognizeRequest): Promise<{ data: OcrPaymentData; raw: Record<string, unknown>; model: string }> {
  const apiKey = process.env.OPENAI_API_KEY; if (!apiKey) throw new Error("尚未配置 OPENAI_API_KEY，无法执行支付截图识别")
  const model = process.env.PAYMENT_OCR_MODEL || process.env.OPENAI_MODEL || "gpt-4.1-mini"; const baseUrl = (process.env.OPENAI_BASE_URL || "https://api.openai.com/v1").replace(/\/$/, "")
