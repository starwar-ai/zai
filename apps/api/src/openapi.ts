  ...
  components: {
    ...
    schemas: {
      ...
      ExternalInvoiceRecognizeResult: {
        type: "object", required: ["recognitionId", "originalFilename", "items"], properties: { recognitionId: { type: "string", format: "uuid", description: "服务端识别记录 ID" }, originalFilename: { type: "string" }, clientRequestId: { type: "string" }, extractionMethod: { type: "string", enum: ["QR", "AI", "HYBRID"], description: "QR=二维码、AI=模型、HYBRID=二维码核心字段加模型补全" }, invoiceType: { type: "string", enum: ["VAT_NORMAL", "VAT_SPECIAL"], description: "VAT_NORMAL=普通发票（包括通行费电子发票），VAT_SPECIAL=增值税专用发票；无法明确识别时省略" }, invoiceCategory: { type: "string", enum: ["STANDARD", "TOLL"], description: "STANDARD=标准商品/服务发票，TOLL=通行费电子发票" }, invoiceNumber: { type: "string" }, invoiceDate: { type: "string", description: "优先返回 YYYY-MM-DD；无法标准化时保留票面格式" }, buyerName: { type: "string" }, buyerTaxId: { type: "string" }, sellerName: { type: "string" }, sellerTaxId: { type: "string" }, subtotal: { type: "string", description: "金额合计（不含税）" }, totalTax: { type: "string" }, totalAmount: { type: "string", description: "价税合计" }, totalAmountInWords: { type: "string" }, remarks: { type: "string", description: "备注栏原文；票面为空时字段可能省略" }, drawer: { type: "string", description: "开票人" }, vehiclePlate: { type: "string", description: "通行费发票车牌号" }, vehicleType: { type: "string", description: "通行费发票车辆类型" }, tollAmount: { type: "string", description: "通行费金额" }, tollDate: { type: "string", description: "通行日期" }, items: { type: "array", description: "通行费发票可为空", items: { $ref: "#/components/schemas/InvoiceItem" } } },
      },
