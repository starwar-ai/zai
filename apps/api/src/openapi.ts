  ...
  components: {
    ...
    schemas: {
      ...
      ExternalInvoiceRecognizeResult: {
        type: "object", required: ["recognitionId", "originalFilename", "items"], properties: { recognitionId: { type: "string", format: "uuid", description: "服务端识别记录 ID" }, originalFilename: { type: "string" }, clientRequestId: { type: "string" }, extractionMethod: { type: "string", enum: ["QR", "AI", "HYBRID"], description: "QR=二维码、AI=模型、HYBRID=二维码核心字段加模型补全" }, invoiceNumber: { type: "string" }, invoiceDate: { type: "string", description: "优先返回 YYYY-MM-DD；无法标准化时保留票面格式" }, buyerName: { type: "string" }, buyerTaxId: { type: "string" }, sellerName: { type: "string" }, sellerTaxId: { type: "string" }, subtotal: { type: "string", description: "金额合计（不含税）" }, totalTax: { type: "string" }, totalAmount: { type: "string", description: "价税合计" }, totalAmountInWords: { type: "string" }, remarks: { type: "string", description: "备注栏原文；票面为空时字段可能省略" }, drawer: { type: "string", description: "开票人" }, items: { type: "array", minItems: 1, items: { $ref: "#/components/schemas/InvoiceItem" } } },
      },
