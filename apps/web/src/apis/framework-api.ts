...
export const api = {
  ...
  invoiceOcrModel: () => request<OcrModelInfo>("/api/ocr/model"),
  ...
  ocrRecognition: (type: OcrRecognitionType, id: string) => request<OcrRecognitionRecord>(`/api/ocr/recognitions/${id}?recognitionType=${type}`),
  ...
  updateBusinessCard: (id: string, input: OcrBusinessCardUpdateRequest) => request<OcrRecognitionRecord>(`/api/ocr/business-cards/${id}`, { method: "PUT", body: JSON.stringify(input) }),
