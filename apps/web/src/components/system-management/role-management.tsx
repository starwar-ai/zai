...
function permissionGroups(menus: SystemMenuRecord[]): PermissionGroup[] {
  ...
  const imageSearch = [{ code: "image-search:view", label: "查看图库与个人历史" }, { code: "image-search:search", label: "执行以图搜图" }, { code: "image-search:manage", label: "维护公共图库" }, { code: "image-search:index", label: "生成图片索引" }]
  return [{ id: "system", label: "系统管理", items: unique(systemItems) }, { id: "navigation", label: "页面访问", items: unique(navigation) }, { id: "documents", label: "单据操作", items: unique(documents) }, { id: "ocr", label: "OCR 识别", items: ocr }, { id: "image-search", label: "以图搜图", items: imageSearch }]
}
