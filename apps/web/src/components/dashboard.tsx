...
import { CustomerResearchQueueWidget } from "@/components/customer-research-queue-widget"
...
export function Dashboard({ data, schemas, onOpenDocument, onOpenList, visibleWidgets }: DashboardProps) {
  const flowSchemas = schemas.filter((schema) => schema.includeInBusinessFlow !== false)
  const metrics = [
    { label: "全部单据", value: data.totalDocuments, note: `覆盖 ${schemas.length} 个业务模块`, icon: FileStack, tone: "teal" },
  ...
  return <>
    ...
    <div className="dashboard-grid">
      {visibleWidgets.includes("customer-research-queue") && <CustomerResearchQueueWidget onOpenList={onOpenList} />}
      {visibleWidgets.includes("recent-documents") && <Card className="span-2"><div className="panel-header"><div><h2>最近单据</h2><p>跨业务模块的最新进展</p></div><button className="text-button" onClick={() => onOpenList(schemas[0]?.typeId)}>查看全部 <ArrowRight /></button></div>
        <div className="data-table"><div className="table-row table-head"><span>单据编号</span><span>业务类型</span><span>往来单位 / 主题</span><span>状态</span><span>更新时间</span></div>
          {data.recentDocuments.map((document) => { const schema = schemas.find((item) => item.typeId === document.typeId); const subject = String(document.masterData.customerName || document.masterData.companyName || document.masterData.planName || document.masterData.supplierName || "—"); return <button className="table-row" key={document.id} onClick={() => onOpenDocument(document)}><strong>{document.code}</strong><span>{schema?.typeName}</span><span className="truncate">{subject}</span><StatusPill status={document.status} label={schema?.statusLabels?.[document.status]} /><span>{formatDate(document.updatedAt)}</span></button> })}
        </div>
      </Card>}
      ...
      {visibleWidgets.includes("business-flow") && <Card className="span-2"><div className="panel-header"><div><h2>业务链路</h2><p>从客户需求到仓库收货的可追溯流程</p></div></div><div className="process-flow">{flowSchemas.map((schema, index) => <div key={schema.typeId}><button onClick={() => onOpenList(schema.typeId)}><span>0{index + 1}</span><strong>{schema.typeName}</strong><small>{data.typeCounts.find((item) => item.typeId === schema.typeId)?.count || 0} 张单据</small></button>{index < flowSchemas.length - 1 && <ArrowRight />}</div>)}</div></Card>}
