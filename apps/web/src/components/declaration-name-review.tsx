...
export function DeclarationNameReview() {
  ...
  const [message, setMessage] = useState<{ variant: "success" | "danger" | "info"; text: string; detail?: string } | null>(null)
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { void api.declarationNameJob(job.id).then((next) => {
      ...
      if (["COMPLETED", "FAILED"].includes(next.status)) {
        setMessage({
          variant: next.status === "COMPLETED" ? "success" : "danger",
          text: `生成任务已结束：成功 ${next.successCount}，待复核 ${next.reviewCount}，失败 ${next.failedCount}`,
          ...(next.status === "FAILED" ? { detail: next.errorMessage || "该任务未记录错误详情。" } : {}),
        })
        void load()
      }
  ...
  return <div className="declaration-name-page">
    ...
    {message && <Alert variant={message.variant}><span>{message.text}</span>{message.detail && <Button className="declaration-error-detail-button" size="sm" variant="ghost" onClick={() => setErrorDetailOpen(true)}>详情</Button>}</Alert>}
    ...
    <Dialog open={errorDetailOpen} title="大模型返回错误详情" description="以下内容用于排查模型响应格式或字段校验问题。" width={760} onClose={() => setErrorDetailOpen(false)} footer={<Button onClick={() => setErrorDetailOpen(false)}>关闭</Button>}><pre className="declaration-error-detail">{message?.detail}</pre></Dialog>
  </div>
}
