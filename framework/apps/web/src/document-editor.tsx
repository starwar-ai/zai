...
import { pluginRegistry, renderExtraTab, useSchemaEffects } from "./plugin-registry"
...
function actionClass(action: FormActionDefinition): string {
  if (action.variant === "success") return "success-button"
  if (action.variant === "danger") return "danger-ghost-button"
  if (action.variant === "secondary") return "secondary-button"
...
export function DocumentEditor({ documentId, schemas, onBack, onOpen, onChanged, onDirtyChange }: EditorProps) {
  ...
  return <>
    ...
    <div className="editor-tabs"><button className={tab === "form" ? "active" : ""} onClick={() => setTab("form")}>单据内容</button><button className={tab === "trace" ? "active" : ""} onClick={() => setTab("trace")}><GitBranch />来源与下推</button><button className={tab === "history" ? "active" : ""} onClick={() => setTab("history")}><FileClock />操作记录 <span>{activities.length}</span></button>{schema.extraTabs?.map((extraTab) => <button className={tab === `extra:${extraTab.id}` ? "active" : ""} onClick={() => setTab(`extra:${extraTab.id}`)} key={extraTab.id}>{extraTab.label}</button>)}</div>
