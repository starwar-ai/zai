...
export function Dashboard({ data, schemas, onOpenDocument, onOpenList, visibleWidgets }: DashboardProps) {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">2026 年 7 月 18 日 · 星期六</p><h1>早上好，林默</h1><p>这里是今天的业务概览，有 <strong>{data.pendingApprovals}</strong> 项审批等待处理。</p></div><button className="primary-button"><CircleDollarSign size={17} />查看经营分析</button></div>
    ...
    <div className="dashboard-grid">
      {visibleWidgets.includes("recent-documents") && <article className="panel span-2"><div className="panel-header"><div><h2>最近单据</h2><p>跨业务模块的最新进展</p></div><button className="text-button" onClick={() => onOpenList(schemas[0]?.typeId)}>查看全部 <ArrowRight /></button></div>
      ...
      {visibleWidgets.includes("business-distribution") && <article className="panel"><div className="panel-header"><div><h2>业务分布</h2><p>各类单据当前数量</p></div></div><div className="distribution-list">{data.typeCounts.map((item, index) => <button key={item.typeId} onClick={() => onOpenList(item.typeId)}><span className={`dot dot-${index + 1}`} /><div><p><strong>{item.typeName}</strong><b>{item.count}</b></p><div className="progress"><i style={{ width: `${Math.max((item.count / maxCount) * 100, 8)}%` }} /></div></div></button>)}</div></article>}
      {visibleWidgets.includes("business-flow") && <article className="panel span-2"><div className="panel-header"><div><h2>业务链路</h2><p>从客户需求到仓库收货的可追溯流程</p></div></div><div className="process-flow">{schemas.map((schema, index) => <div key={schema.typeId}><button onClick={() => onOpenList(schema.typeId)}><span>0{index + 1}</span><strong>{schema.typeName}</strong><small>{data.typeCounts.find((item) => item.typeId === schema.typeId)?.count || 0} 张单据</small></button>{index < schemas.length - 1 && <ArrowRight />}</div>)}</div></article>}
      {visibleWidgets.includes("recent-activities") && <article className="panel"><div className="panel-header"><div><h2>最近动态</h2><p>团队业务操作记录</p></div></div><div className="activity-list">{data.recentActivities.slice(0, 5).map((activity) => <div key={activity.id}><span className="activity-avatar">{activity.operator.slice(0, 1)}</span><p><strong>{activity.operator}</strong><span>{activity.message}</span><small>{formatDate(activity.createdAt, true)}</small></p></div>)}</div></article>}
    </div>
  </>
}
