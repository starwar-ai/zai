...
import { Bell, Boxes, CircleHelp, FileText, LayoutDashboard, Menu, PackageOpen, PanelLeftClose, Search, Settings, ShoppingCart, Sparkles, Warehouse } from "lucide-react"
...
import type { WorkspaceTab, WorkspaceView } from "./workspace-types"

const menuIcons = { LayoutDashboard, FileText, PackageOpen, ShoppingCart, Warehouse, Settings, CircleHelp }
...
export function App() {
  ...
  const renderTab = (tab: WorkspaceTab) => {
    ...
    return <div className="panel help-page"><CircleHelp /><h1>应用外壳帮助</h1><p>使用顶部标签在多个业务页面之间切换。右键标签可以刷新、关闭当前、关闭其他或关闭全部；有未保存内容时系统会先提示确认。</p></div>
  }
  ...
  return <div className={`app-shell ${sidebarOpen ? "" : "sidebar-collapsed"} ${settings.compactMode ? "compact-shell" : ""} ${settings.showGlobalStatusBar ? "has-status-bar" : ""}`}>
    ...
    <main className="main-area"><header className="topbar"><div className="topbar-left"><button className="icon-button" onClick={() => setSidebarOpen((value) => !value)}>{sidebarOpen ? <PanelLeftClose /> : <Menu />}</button><div className="breadcrumb"><span>{shell.config.appName}</span><b>/</b><strong>{activeTab.title}</strong></div></div><div className="topbar-actions"><div className="global-search"><Search /><input placeholder="搜索单据、客户、产品..." /><kbd>⌘ K</kbd></div><button className="icon-button notification" onClick={() => setNotificationsOpen((value) => !value)} title="通知中心"><Bell />{unreadCount > 0 && <i />}</button><button className="avatar small avatar-button" onClick={() => openView({ kind: "settings" }, "用户设置", "settings")}>{shell.user.name.slice(0, 1)}</button></div></header>
