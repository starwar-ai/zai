...
│   │   ├── src/schemas.ts   # 单据元数据与流程规则
│   │   ├── src/database.ts  # Prisma 数据库连接
│   │   └── src/             # API 与事务化领域服务
│   └── web/                 # React + Vite 管理端
│       └── src/             # 工作台、列表、动态表单、追溯页面
├── packages/
│   └── shared/              # 前后端共享类型和状态常量
...
- 权限菜单：菜单组和权限码由后端外壳配置声明，前端按当前用户权限生成侧边栏
...
1. 在 `apps/api/src/schemas.ts` 增加一个 `DocumentSchema`。
...
前端插件注册入口位于 `apps/web/src/plugin-registry.tsx`：
